...
s = ensure_import(cyber.read_text())
...
if '[System] output=%@' not in s:
    ...
    if anchor in s:
        s = s.replace(anchor, anchor + '\n    HFACyberUIAppendLog([NSString stringWithFormat:@"[System] output=%@", HFAOutputDirectory()]);', 1)
cyber.write_text(s)

app = SRC / 'HFAMapAppLocalResolver.m'
s = app.read_text()
s, count = re.subn(r'@"HFAMap(?:Universal)?[^\n"]*AppLocalMenuResolver"',
                   '@"HFAMap RuntimeAnalyzer v0.3.2 AutoBackend"', s, count=1)
...
if 'src/HFAMapOutputPaths.m' not in make:
    ...
    if marker not in make: raise SystemExit('Makefile source anchor missing')
    make = make.replace(marker, marker + ' src/HFAMapOutputPaths.m', 1)
MAKEFILE.write_text(make)

# First-stage checks only. The final path audit runs after the safe-discovery
# patch, which also handles multiline and C-style legacy log paths.
ui = cyber.read_text()
scan_start = ui.find('- (void)actionCyberScan:')
scan_end = ui.find('- (void)actionCyberExport:', scan_start)
if scan_start < 0 or scan_end < 0: raise SystemExit('UI scan/export actions not found')
scan_body = ui[scan_start:scan_end]
if 'HFAAppLocalScanCandidates()' not in scan_body: raise SystemExit('baseline discovery call missing')
...
if 'HFAMap RuntimeAnalyzer v0.3.2 AutoBackend' not in ui: raise SystemExit('v0.3.2 UI version missing')
...
if 'src/HFAMapOutputPaths.m' not in MAKEFILE.read_text(): raise SystemExit('output helper missing from Makefile')
