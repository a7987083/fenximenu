...
static void ZPRegister(NSString *identifier,NSString *label,NSString *source,NSString *className){
    if(!identifier.length||!label.length)return; NSDictionary *r=@{@"identifier":identifier,@"label":label,@"key":[identifier hasSuffix:@"-switch"]?identifier:[identifier stringByAppendingString:@"-switch"],@"source":source?:@"?",@"class":className?:@"?"};
    pthread_mutex_lock(&gZPFeatureLock); if(!gZPFeatures)gZPFeatures=[NSMutableDictionary dictionary]; if(!gZPFeatures[identifier])gZPFeatures[identifier]=r; pthread_mutex_unlock(&gZPFeatureLock);
}
static void ZPInspectControl(UIControl *control){
    NSString *label=ZPLabel(control); if(!label.length)return;
    NSString *identifier=ZPStringGetter(control,"identifier"); if(identifier.length)ZPRegister(identifier,label,@"control",NSStringFromClass(object_getClass(control)));
    NSSet *targets=control.allTargets; NSUInteger seen=0;
    for(id target in targets){ if(++seen>64)break; NSString *tid=ZPStringGetter(target,"identifier"); if(tid.length)ZPRegister(tid,label,@"control-target",NSStringFromClass(object_getClass(target))); }
}
