...
static const NSUInteger kMaxViews=2048,kMaxTargets=128,kMaxContainers=256,kMaxChildren=64,kMaxFields=48;
...
static NSDictionary *ZPObjectDictionary(id o){
    ...
    for(Class c=object_getClass(o);c&&c!=NSObject.class;c=class_getSuperclass(c)){
        unsigned count=0;Ivar *iv=class_copyIvarList(c,&count);
...
static NSDictionary *ZPFieldEvidence(id object){
    NSMutableArray *fields=[NSMutableArray array];Class cls=object_getClass(object);
    for(Class c=cls;c&&c!=NSObject.class&&fields.count<kMaxFields;c=class_getSuperclass(c)){
        unsigned count=0;Ivar *iv=class_copyIvarList(c,&count);
    ...
    return @{@"class":NSStringFromClass(cls)?:@"?",@"instanceSize":@(class_getInstanceSize(cls)),@"offsetSemantics":@"objc-instance-ivar-only",@"fields":fields};
}
NSDictionary *ZPContainerCaptureSeeds(NSString *candidateImage,NSTimeInterval deadline){
    ...
    return @{@"status":NSDate.date.timeIntervalSince1970>deadline?@"timeout":@"complete",@"seeds":seeds,@"metrics":@{@"views":@(views),@"targets":@(targets.count)}};
}
NSDictionary *ZPContainerResolve(NSString *candidateImage,NSDictionary *snapshot,NSTimeInterval deadline){
    ...
    for(NSUInteger cursor=0;cursor<containers.count&&cursor<kMaxContainers&&NSDate.date.timeIntervalSince1970<=deadline;cursor++){
        ...
        NSString *label=ZPStringValue(ZPValueForAliases(d,@[@"label",@"title",@"name",@"displayname"]));NSString *identifier=ZPStringValue(ZPValueForAliases(d,@[@"identifier",@"id"]));
        NSString *containerNode=ZPEvidenceGraphAddNode(@"container",@{@"class":NSStringFromClass(object_getClass(object))?:@"?",@"pointer":ZPPtr(object),@"image":ZPImageForObject(object)?:@""});NSString *parent=containers[cursor][@"targetNode"];if([parent isKindOfClass:NSString.class])ZPEvidenceGraphAddEdge(parent,@"reaches",containerNode,@{@"strength":@"structural"});
