...
static BOOL HFAMapInstallFloatingUI(void)
{
    ...
    title.text = @"HFAMap v2.4.6-dev Decoupled";
    ...
    status.text = @"1 Search = full-list lightweight discovery\n2 Analyze = selected menu only\n3 Probe = exact runtime control/action";
    ...
    NSLog(@"[HFAMap] v2.4.6 decoupled UI installed on window=%@", window);
    return YES;
}
