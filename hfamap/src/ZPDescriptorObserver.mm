...
static NSMutableArray<NSDictionary *> *gZPObserverEvents;
...
static void ZPAppendEvent(id object, SEL selector, NSDictionary *input) {
    ...
    BOOL armed = gZPObservedClass && object_getClass(object) == gZPObservedClass && NSDate.date.timeIntervalSince1970 <= gZPObserverDeadline;
    pthread_mutex_unlock(&gZPObserverLock);
    if (!armed) return;
    NSDictionary *event=@{ @"schema":@"com.hfa.zpatchig.descriptor-event/v1", @"time":@(NSDate.date.timeIntervalSince1970), @"appIdentity":ZPAppIdentity(), @"image":gZPObservedImage?:@"?", @"class":NSStringFromClass(object_getClass(object))?:@"?", @"descriptor":ZPPtr((__bridge const void *)object), @"selector":NSStringFromSelector(selector), @"input":input?:@{}, @"snapshot":ZPSnapshot(object)?:@{} };
    ...
    pthread_mutex_lock(&gZPObserverLock);
    if(!gZPObserverEvents)gZPObserverEvents=[NSMutableArray array];
    if(gZPObserverEvents.count<kZPMaxObserverEvents)[gZPObserverEvents addObject:event];
    pthread_mutex_unlock(&gZPObserverLock);
    HFADiagnosticsLog(@"descriptor-observer",@"event",@{ @"selector":NSStringFromSelector(selector), @"descriptor":event[@"descriptor"], @"snapshot":event[@"snapshot"] });
}
...
static BOOL ZPHookSetter(Class cls, const char *name, IMP replacement, IMP *slot, NSString **why) {
    Method m=class_getInstanceMethod(cls,sel_registerName(name)); if(!m){if(why)*why=@"missing-method";return NO;}
    unsigned argc=method_getNumberOfArguments(m); if(argc<3){if(why)*why=@"bad-arity";return NO;}
...
static void ZPRestoreSetter(Class cls,const char *name,IMP replacement,IMP orig){ if(!cls||!orig)return; Method m=class_getInstanceMethod(cls,sel_registerName(name)); if(!m)return; if(method_getImplementation(m)==replacement)method_setImplementation(m,orig); }

void ZPDescriptorObserverDisarm(NSString *reason) {
    pthread_mutex_lock(&gZPObserverLock); Class cls=gZPObservedClass; gZPObservedClass=Nil; gZPObserverDeadline=0; pthread_mutex_unlock(&gZPObserverLock);
    if(!cls)return;
    ...
    HFADiagnosticsLog(@"descriptor-observer",@"disarmed",@{ @"reason":reason?:@"manual", @"class":NSStringFromClass(cls)?:@"?" });
}
...
NSDictionary *ZPDescriptorObserverArm(NSDictionary *candidate, NSDictionary *descriptor, NSTimeInterval duration) {
    ...
    duration=MAX(1.0,MIN(duration,30.0)); NSMutableArray *hooked=[NSMutableArray array]; NSMutableDictionary *skipped=[NSMutableDictionary dictionary]; NSString *why=nil;
    if(ZPHookSetter(cls,"setIdentifier:",(IMP)ZPHookObject,&gOrigIdentifier,&why))[hooked addObject:@"setIdentifier:"]; else if(why)skipped[@"setIdentifier:"]=why; why=nil;
    if(ZPHookSetter(cls,"setOffset:",(IMP)ZPHookObject,&gOrigOffset,&why))[hooked addObject:@"setOffset:"]; else if(why)skipped[@"setOffset:"]=why; why=nil;
    if(ZPHookSetter(cls,"setSignature:",(IMP)ZPHookObject,&gOrigSignature,&why))[hooked addObject:@"setSignature:"]; else if(why)skipped[@"setSignature:"]=why; why=nil;
    if(ZPHookSetter(cls,"setRange:",(IMP)ZPHookU64,&gOrigRange,&why))[hooked addObject:@"setRange:"]; else if(why)skipped[@"setRange:"]=why; why=nil;
    if(ZPHookSetter(cls,"setActive:",(IMP)ZPHookBool,&gOrigActive,&why))[hooked addObject:@"setActive:"]; else if(why)skipped[@"setActive:"]=why;
    if(!hooked.count)return @{ @"status":@"not-armed",@"reason":@"no-supported-setters",@"skipped":skipped };
    pthread_mutex_lock(&gZPObserverLock); gZPObservedClass=cls; gZPObservedImage=[candidate[@"image"] copy]; gZPObserverDeadline=NSDate.date.timeIntervalSince1970+duration; gZPObserverEvents=[NSMutableArray array]; pthread_mutex_unlock(&gZPObserverLock);
    NSDictionary *result=@{ @"status":@"armed",@"class":className,@"image":candidate[@"image"]?:@"?",@"durationMs":@((NSUInteger)(duration*1000.0)),@"hookedSetters":hooked,@"skippedSetters":skipped,@"logFile":ZPLogFilename(@"DescriptorEvents.jsonl") };
