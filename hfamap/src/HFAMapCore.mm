...
void HFAMapRunBoundedScan(void (^completion)(NSDictionary *summary)){
 ...
 dispatch_async(ZPWorker(),^{NSTimeInterval started=NSDate.date.timeIntervalSince1970;ZPEvidenceGraphReset();ZPFeatureResolverReset();NSString*session=HFADiagnosticsBeginSession();NSMutableArray*events=[NSMutableArray array];NSDictionary*appIdentity=ZPAppIdentity();[events addObject:@{@"time":@(started),@"stage":@"scan",@"status":@"start",@"version":@"zpatchig-v0.4.2",@"budgetMs":@6000,@"appIdentity":appIdentity}];NSArray*candidates=ZPDiscoverMenuImages(started+2.0,events);NSString*reason=nil;NSDictionary*selected=ZPSelect(candidates,&reason);NSDictionary*analysis=nil;
 if(!selected){analysis=@{@"schema":@"com.hfa.zpatchig.analysis/v4",@"version":@"0.4.2",@"status":@"incomplete",@"session":session,@"appIdentity":appIdentity,@"reason":reason?:@"selection-failed",@"candidates":candidates,@"uiFeatures":@[],@"containerFeatures":@[],@"descriptorEvidence":@[],@"observations":@[],@"validatedFeatures":@[],@"evidenceGraph":ZPEvidenceGraphSnapshot()};}
 else{
   ...
   NSDictionary*descriptor=resolved[@"descriptor"]?:@{};NSDictionary*observer=descriptor.count?ZPDescriptorObserverArm(selected,descriptor,10.0):@{@"status":@"not-armed",@"reason":@"descriptor-missing"};[events addObject:@{@"time":@(NSDate.date.timeIntervalSince1970),@"stage":@"descriptor-observer",@"status":observer[@"status"]?:@"?",@"details":observer}];NSDictionary *graph=ZPEvidenceGraphSnapshot();analysis=@{@"schema":@"com.hfa.zpatchig.analysis/v4",@"version":@"0.4.2",@"status":resolved[@"status"]?:@"analysis-only",@"session":session,@"appIdentity":appIdentity,@"candidate":selected,@"family":resolved[@"family"]?:@"unknown",@"descriptor":descriptor,@"uiFeatures":uiFeatures,@"containerFeatures":containerFeatures,@"descriptorEvidence":descriptorEvidence,@"containerMetrics":containerResult[@"metrics"]?:@{},@"descriptorObserver":observer,@"observations":resolved[@"observations"]?:@[],@"validatedFeatures":resolved[@"validatedFeatures"]?:@[],@"canonicalEligible":resolved[@"canonicalEligible"]?:@NO,@"canonicalReason":resolved[@"canonicalReason"]?:@"unknown",@"evidenceGraph":graph};ZPWriteJSON(graph,@"EvidenceGraph.json");}
